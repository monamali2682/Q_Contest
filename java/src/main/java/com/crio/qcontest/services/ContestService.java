...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        Contest c = contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException("Contest with Id: "+ contestId +" not found!"));
        if(c.getContestStatus().equals(ContestStatus.ENDED)){
            throw new RuntimeException("Contest has been ended!");
        }
        if(c.getContestStatus().equals(ContestStatus.IN_PROGRESS)){
            throw new RuntimeException("Contest has been started already!");
        }
        Contestant ct = new Contestant(user, c);
        contestantRepository.save(ct);
        return ct;
    }
    ...
    public String deleteContestant(Long contestId, String userName) {
        ...
        if(contestant.getUser().getId()==contestant.getContest().getCreator().getId()){
            throw new RuntimeException("Creator can not withdraw from the contest");
        }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        Contest c = contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException("Contest: "+contestId+" not found!"));
        ContestStatus conteststatus = c.getContestStatus();
        if(conteststatus.equals(ContestStatus.IN_PROGRESS)){
            throw new RuntimeException("History for Contest Id: "+ contestId +" not found!");
        }
        if(conteststatus.equals(ContestStatus.NOT_STARTED)){
            throw new RuntimeException("History for Contest Id: "+ contestId +" not found!");
        }
        List<Contestant> contestants = contestantRepository.findAllByContestId(contestId);
        return contestants.stream().sorted( (c1,c2) -> c2.getCurrentContestPoints()-c1.getCurrentContestPoints()).collect(Collectors.toList());
    }  
}
